...
List<OpOrdertickets> pplist=frameMap.get("pplist")!=null?(List<OpOrdertickets>)frameMap.get("pplist"):null;
...
String fd=fdate!=null ?sdf.format(fdate):null;
String ft=opo!=null?opo.getFlyTime():null;
String act=(String)request.getAttribute("action");
String action="";
 if("2".equals(act)){
	action = "commandCenter.do?method=toMainPageContain11&orderdate="+flightDate;
}else if("3".equals(act)){
	action = "commandCenter.do?method=toMainPageContain12&orderdate="+flightDate;
}else if("1".equals(act)){
	action = "commandCenter.do?method=toMainPageContain10&orderdate="+flightDate;
}else{
...
<html>
  <head>
    ...
    <title>My JSP 'CCContainPage1.jsp' starting page</title>
	...
	window.name="wode";
		function shouPiao(){
			parent.document.getElementById("contain1").src="<%=request.getContextPath()%>/commandCenter.do?method=toMainPageContain10&orderdate=<%=flightDate %>";
		}
		
		function huanPai(){
			parent.document.getElementById("contain1").src="<%=request.getContextPath()%>/commandCenter.do?method=toMainPageContain11&orderdate=<%=flightDate %>";
		}
		
		function jinCang(){
			parent.document.getElementById("contain1").src="<%=request.getContextPath()%>/commandCenter.do?method=toMainPageContain12&orderdate=<%=flightDate %>";
		}
		</script>
  </head>
  ...
  <body background="<%=request.getContextPath()%>/image/bg.jpg">
   ...
   <%if(pplist!=null && pplist.size()>0){ %>
    <FONT style="font-size:20px;text-shadow:Red;font-family:'黑体';">起飞时间：</font><FONT style="font-size:20px;text-shadow:Red;font-family:'黑体';color: #b22222"><%=fd %>&nbsp;<%=ft %> 星期 <%=weeks%></font>
    ...
    <%	if (pplist != null)
    	...
    	%>
    	 <tr bgcolor="#Ffffff">
    		...
    		<td align="center" bordercolor="black"><%=status==2?"已售票":status==3?"已换登机牌" :status==4?"已登机":"已退票"%></td>
    	</tr>
    	<%
  	  } 
    %>
