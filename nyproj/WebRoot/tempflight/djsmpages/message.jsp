  ...
  function mydata(){
    ...
    var re = /^[0-9]*$/; 
  	if (txm.value!=null && txm.value !='' && re.test(txm.value) &&parseFloat(txm.value) >idInitVal)    {
  		 ...
  		 UpdateStatusDWR.upTempFly(txm.value,'<%=flightinfoId%>',onHandleM);
	 }else{
	    ...
	    document.getElementById("txmv").focus();
			...
			function onHandleM(data){
				if(data>0){
					var message=document.getElementById("info");
					parent.document.getElementById("uncharge").src="<%=request.getContextPath()%>/tfClientAction.do?method=toFlyAlrSafeCheck&flightinfoId=<%=flightinfoId %>";
					parent.document.getElementById("charged").src="<%=request.getContextPath()%>/tfClientAction.do?method=areadyToFly&flightinfoId=<%=flightinfoId %>&id="+data;
					 message.innerHTML="";
					document.getElementById("txmv").value='';
				      document.getElementById("txmv").focus();					
				}else{
