  ...
  <head>
	...
	<script type="text/javascript">
		function xxInfo(data){
				var url = "<%=request.getContextPath()%>/tfClientAction.do?method=toXxinfoPage&id="+data+"&fid="+document.getElementById("fid").value;
...
function Hide(divid) {
 ...
 <form action="<%=request.getContextPath()%>/tfClientAction.do?method=toLuggInfo" method="post">
   <input id="fid" type="hidden" name="fid" value="<%=request.getAttribute("flightinfoId")==null?"":Integer.valueOf(request.getAttribute("flightinfoId").toString()) %>"/>
    <table border="0" width="1024" align="center">
    	<tr>
    		<th align="right" style="color: #FFFFFF">日期：</th>
    		<td>
    			<table border="0">
					<td width="70%" >
						   <input value="<%=date %>" id="orderdate" name="orderdate" width="100%" readonly="readonly">     
						          </td>
				     <td align="left">
...
<table width="1024" border="0" align="center"  cellpadding="0" cellspacing="1" bgcolor="#3366FF">
  ...
  <tr onclick="xxInfo(${item.id})" bgcolor="#FFFFFF" onmouseout="this.bgColor='#FFFFFF'" onmouseover="this.bgColor='ffcccc'">
     ...
     <td align="center">${item.flightInfo}</td>
