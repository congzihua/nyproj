...
<html>
  <head>
	...
	function dgpage(data,status){
		idValue = data;
  		TfSysmanagerDWR.getOrderStatus(data,status,dgpageHandle);	
		
	}
	function dgpageHandle(data){
		if(data){
			var url = "<%=request.getContextPath()%>/tfDJPAction.do?method=toDJPPage&id="+idValue+"&flightInfoId="+fIds;
	...
	var fIds = '${flighInfo.id}';
	function dgpageHandle1(data){
		if(data){
			var url = "<%=request.getContextPath()%>/tfDJPAction.do?method=toDJPPage1&id="+idValue+"&flightinfoId="+fIds;
			//window.open(url);
	...
	function dgpage1(data,status){
		idValue = data;
  		TfSysmanagerDWR.getOrderStatus(data,status,dgpageHandle1);	
	...
	function toGetOrderInfo(certNo) {
		TfSysmanagerDWR.getOrderIdAndType(fIds,certNo,2,getOrderIdAndTypeHandle);
	}
...
function Hide(divid) {
  ...
  <form action="<%=request.getContextPath()%>/tfDJPAction.do?method=toList" method="post">&nbsp; 
  ...
  <input type="hidden" name="flightNo" value="${flightNo}"/> 
   <input type="hidden" name="flightinfoId" value="${flighInfo.id}"/> 
  <%
 ...
 <table id="txtBox" width="98%" border="0" align="center"  cellpadding="0" cellspacing="1" bgcolor="#3366FF">
  ...
  <tr bgcolor="#FFFFFF" onmouseout="this.bgColor='#FFFFFF'" onmouseover="this.bgColor='ffcccc'" style="height:30px; border-color:#333333; text-align:center; vertical-align:middle;	word-break:break-all;overflow:auto;">
     <td align="center" width="5%"><%=++i %></td>
      <td align="center" width="5%">${item.flightInfo}</td>
