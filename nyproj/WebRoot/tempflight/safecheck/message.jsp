  ...
  function mydata(){
    ...
    var re = /^[0-9]*$/; 
  	if (txm.value!=null && txm.value !='' && re.test(txm.value)&&parseFloat(document.getElementById("txmv").value)>idInitVal)    {
  		 ...
  		 UpdateStatusDWR.updateTempStatusAtDJP(txm.value,<%=flightinfoId%>,onHandleM);
			...
			function onHandleM(data){
				if(data>0){
					var message=document.getElementById("info");
				   parent.document.getElementById("uncharge").src="<%=request.getContextPath()%>/tfClientAction.do?method=toPageAtDJP1&flightinfoId=<%=flightinfoId%>";
					parent.document.getElementById("charged").src="<%=request.getContextPath()%>/tfClientAction.do?method=toPageAtDJP2&flightinfoId=<%=flightinfoId%>&id="+data;
					message.innerHTML="";
				    document.getElementById("txmv").value='';
				      document.getElementById("txmv").focus();
				}else{
