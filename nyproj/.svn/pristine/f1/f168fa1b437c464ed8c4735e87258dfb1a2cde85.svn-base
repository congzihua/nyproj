...
<%
	...
	if (list == null) {
		out.println("<font size=12 color=\"red\">查询模板错误</font>");
		return;
	}
	Iterator iter = list.iterator();
	if (list.size() == 0) {
		out.println("<font size=12 color=\"red\">请先添加模板</font>");
		return;
	}*/
%>
...
<html>
	...
	<body style="margin:0px"
		...
		<html:form action="/gwlzadmin/doctypes/docTypesAction.do?oper=add"
			method="post">
			<html:hidden property="templetid" name="docTypesForm" />
			<html:hidden property="dmpid" name="docTypesForm"
				value="<%=request.getParameter("dmpid")%>" />
							...
							<table style="font-size:14px;color:#568ba4" border="0"
								...
								<tr>
									...
									<td>
										...
										<html:text property="typename" name="docTypesForm"></html:text>
