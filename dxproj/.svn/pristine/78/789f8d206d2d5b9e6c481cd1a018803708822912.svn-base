...
//-->
  ...
  <form action="<%=request.getContextPath()%>/clientAction.do?method=saltTaUnits" method="post">
...
<table width="100%" border="1" cellspacing="0" cellpadding="0" frame="void">
	 ...
	 <tr bgcolor="#F0F0F0">
	 	...
	 	<span style="font-size:15px;text-shadow:Red;font-family:'黑体';"><%=request.getAttribute("orderdate")==null||request.getAttribute("orderdate").toString().trim().equals("")?"":request.getAttribute("orderdate").toString() %> ${im}售 票 信 息 统 计</span>
	 	</th>
  ...
  <%int i = 0,nums=0;Double pjxj = 0.0D,total = 0.0D;List<OpOrdertickets> list = request.getAttribute("countList")==null?null:(List<OpOrdertickets>)request.getAttribute("countList"); %>
