...
<%
   ...
   List list = (List)request.getAttribute("unBoundList");
...
<html>
		...
		function bound()
		{
		   ...
		   var nodes = document.getElementsByName("check_box");
		     ...
		     if (idstring == '')
		     {
		      ...
		      window.open('accountieAction.do?oper=updatebound&userid=<%=request.getParameter("userid")%>&idString='+idstring,'bound','top='+(screen.height/2-60)+',left='+(screen.width/2-200)+',width=400,height=120,scrollbars=no,resizable=yes,center:yes');
	...
	<body oncontextmenu="if (!event.ctrlKey){return false;}"
		style="margin:0px">
		<html:form action="/gwlzadmin/accountie/accountieAction.do?oper=unbound"  method="post">
	    <input type="hidden" name="userid" value="<%=request.getParameter("userid")%>"/>
		<table>
			<tr>
				<td>
					<input type="button" class="button2" onclick="javascript:bound()"
						value="绑定" />
				</td>
