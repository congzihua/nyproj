/*
 * Generated by MyEclipse Struts
 * Template path: templates/java/JavaClass.vtl
 */
package com.founder.enp.action;

import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import org.apache.struts.action.Action;
import org.apache.struts.action.ActionForm;
import org.apache.struts.action.ActionForward;
import org.apache.struts.action.ActionMapping;

import com.founder.enp.forms.DocNumOperForm;
import com.founder.enp.service.DocService;

/** 
 * MyEclipse Struts
 * Creation date: 08-09-2009
 * 
 * XDoclet definition:
 * @struts.action
 */
public class DocNumOperAction extends Action {
	/*
	 * Generated Methods
	 */

	/** 
	 * Method execute
	 * @param mapping
	 * @param form
	 * @param request
	 * @param response
	 * @return ActionForward
	 */
	public ActionForward execute(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response) {
		// TODO Auto-generated method stub
		DocNumOperForm frm = (DocNumOperForm)form;
		DocService service = new DocService();
		if (service.docNumOper(frm))
		{
			request.setAttribute("oper", "docNumOpersuccess");
		}
		else
		{
			request.setAttribute("oper", "docNumOpererror");
		}
		return mapping.findForward("tip");
	}
}