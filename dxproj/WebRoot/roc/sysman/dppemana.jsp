...
<html>
	...
	<body style="margin:0px"
		...
		<form name="userForm" action="<%=request.getContextPath()%>/ticketsAllocAction.do?method=seleTicketsallocList"  method="post">
		...
		<table style="margin:0px;font-size:12px" border="0" width="800"
			...
			<tr>
				<td width="95%" height="25">
					<font face="Wingdings" size="2">&nbsp;l </font>当前位置：基础数据管理 → 票额分配管理
				</td>
			</tr>
		...
		<table style="font-size:12px;">
			<tr>
				...
				<td>
					<input type="button" class="button2" onclick="javascript:bentchAdd()"
						value="批量增加" />
				</td>
				<td>
				...
				<td>
					<input type="button" class="button2" onclick="javascript:key()"
						value="解锁"/>
				</td>
				<td>
				...
				<td>
					<input type="button" class="button2" onclick="javascript:fenPei()"
						value="票额分配" />
				</td>
				...
				<td style="color:#568ba4">
					<table border="0">
						        <td width="70%" >
						        <input value="<%=request.getAttribute("seleorderdate")==null?"":request.getAttribute("seleorderdate").toString() %>" id="seleorderdate" name="seleorderdate" width="100%" readonly="readonly">        </td>
						        <td align="left">
			...
			<tr align="middle">
				<td valign="top" align="center">
					...
					<table id="listtable" cellspacing="2" cellpadding="1"
						...
						width="100%">
						<tr align="middle">
							<td background="<%=request.getContextPath()%>/images/bg_board.gif" height=25 width=5%>
						...
						<tr onmouseout="this.bgColor=''"
							...
							<td 
								style="cursor:hand" width="15%">
								<input type="hidden" name="orderdate" value="<fmt:formatDate value="${item.orderdate}" pattern="yyyy-MM-dd"/>"/><fmt:formatDate value="${item.orderdate}" pattern="yyyy-MM-dd"/>
							</td>
							
							<td 
								style="cursor:hand" width="15%">
								<input type="hidden" name="ordertime" value="${item.ordertime==null?"":item.ordertime}"/>${item.ordertime==null?"":item.ordertime}
							</td>
							
							<td 
								style="cursor:hand" width="15%">
								<input type="hidden" name="amount" value="${item.amount}"/>${item.amount}
							</td>
							<td 
								style="cursor:hand" width="15%">
								<input type="hidden" name="lockednum" value="${item.lockednum}"/>${item.lockednum}
							</td>
						</tr>
						<TR>
							<TD background="<%=request.getContextPath()%>/images/dot_board.gif" colSpan=7 height=1></TD>
						</TR>
						</c:forEach>
