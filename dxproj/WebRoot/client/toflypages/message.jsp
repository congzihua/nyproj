  ...
  function mydata(){
    ...
    var re = /^[0-9]*$/; 
  	if (txm.value!=null && txm.value !='' && re.test(txm.value)&&parseFloat(document.getElementById("txmv").value)>idInitVal)    {
  		 ...
  		 UpdateStatusDWR.updateStatusAtDJP(txm.value,fIds,onHandleM);
			...
			function onHandleM(data){
				if(data>0){
					var message=document.getElementById("info");
				   parent.document.getElementById("uncharge").src="<%=request.getContextPath()%>/clientAction.do?method=toPageAtDJP1&orderdate=<%=orderdate %>&flyTime=<%=flyTime%>";
					parent.document.getElementById("charged").src="<%=request.getContextPath()%>/clientAction.do?method=toPageAtDJP2&orderdate=<%=orderdate %>&flyTime=<%=flyTime%>&id="+data;
					message.innerHTML="";
				    document.getElementById("txmv").value='';
				      document.getElementById("txmv").focus();
				}else{
