...
<html>
	<head>
		...
		function order()
		{
		   window.open('order.jsp?dmpid=<%=dmpid%>','del','top='+(screen.height/2-300)+',left='+(screen.width/2-400)+',width=800,height=600,scrollbars=no,resizable=yes,center:yes');
		}
         function transfer()
         {
		...
		function del()
		{
		   ...
		   if (!confirm("用户相关信息将被删除，确认删除吗?"))
		   {
		        return ;
		   }
		  window.open('userAction.do?oper=del&idString='+idstring,'del','top='+(screen.height/2-60)+',left='+(screen.width/2-200)+',width=400,height=120,scrollbars=no,resizable=yes,center:yes');
  
		}
		function add()
		{
	        window.open('add.jsp?dmpid=<%=dmpid%>','新建用户','top='+(screen.height/2-200)+',left='+(screen.width/2-325)+',width=650,height=400,scrollbars=no,resizable=yes,center:yes');
		}
		function edit()
		{
		    ...
		    if (userid == false)
		    return false;
		    window.open('userAction.do?oper=userInfo&userid='+userid,'修改用户','top='+(screen.height/2-200)+',left='+(screen.width/2-325)+',width=650,height=400,scrollbars=no,resizable=yes,center:yes');
	...
	<body style="margin:0px"
	    ...
	    <input type="hidden" name="dmpid" value="<%=dmpid%>"/>
		...
		<table style="margin:0px;font-size:12px" border="0" width="800"
			...
			<tr>
				<td width="95%" height="25">
					<font face="Wingdings" size="2">&nbsp;l </font>当前位置：用户管理 → 用户管理
				</td>
			</tr>
...
<td background="../../images/bg_board.gif" width=10%>
						...
						<%
								...
								Integer order = user.getOrders() == null ? 0:user.getOrders();
						%>
						<tr onmouseout="this.bgColor=''" onmouseover="this.bgColor='ffcccc'" align="middle"
							style="FONT-SIZE: 14px;color:#000000">
							<td width=10%>
								<input onclick="checkSelected()" type="checkbox"
									id="check<%=user.getUserid()%>" name="check_box" />
                               
							</td>
							
							<td onclick="selected(<%=user.getUserid()%>)"
								style="cursor:hand" width=15%>
								<%=user.getAccount()%>
							</td>
							<td onclick="selected(<%=user.getUserid()%>)"
								style="cursor:hand" width="25%">
								<%=user.getName()%>
							</td>
							<td onclick="selected(<%=user.getUserid()%>)"
								style="cursor:hand" width="25%">
							...
							<td width=10%>
							 <select onchange="javascirpt:switchChange(<%=user.getUserid() %>);"  name="orderSelect<%=user.getUserid() %>">
                               <%for (int i = 1;i <= count+1;i++)
                               { %><option <%if (order == i) {%> selected <%} %> value="<%=i %>">
